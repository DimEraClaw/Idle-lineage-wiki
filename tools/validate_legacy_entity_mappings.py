...
BASELINE={
 ("monster_name_to_id","resolved"):460,("drop_owner_to_monster_id","resolved"):433,
 ("wiki_monster_to_monster_id","resolved"):408,("craft_monster_to_monster_id","resolved"):365,
 ("craft_monster_to_monster_id","unresolved"):1,("legacy_card_key_to_monster_id","compatibility_only"):409,
 ("map_label_to_map_id","resolved"):100,("map_label_to_map_id","missing_target"):18,
...
def validate(datasets:list[dict[str,Any]],schema:dict[str,Any],entities:dict[str,set[str]],*,check_baseline=True,raw_bytes:list[bytes]|None=None):
 ...
 for i,r in enumerate(records):
  ...
  if st=="ambiguous" and len(cands)<2:out.append(diag("ambiguous_mapping","ambiguous mapping needs at least two candidates",path))
  if st in ("unresolved","missing_target") and target:out.append(diag("missing_mapping_target",f"{st} mapping cannot have target",path))
  if target:
 ...
 if check_baseline:
  ...
  if sum(1 for r in records if r.get("status")=="ambiguous") or sum(1 for r in records if r.get("status")=="conflict"):out.append(diag("conflicting_mapping","baseline requires zero ambiguous/conflict records"))
 return out
def entity_sets(data):return {"monster":{x[0] for x in data["mobs"]},"map":set(data["maps"]),"item":{x[0] for x in data["items"]},"card":set()}
def main():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument("--resolved",type=Path,default=ROOT/"data/mappings/legacy-entity-mappings.json");p.add_argument("--unresolved",type=Path,default=ROOT/"data/mappings/unresolved-legacy-mappings.json");p.add_argument("--schema",type=Path,default=DEFAULT_SCHEMA);p.add_argument("--source-root",type=Path,default=ROOT);a=p.parse_args()
 try:
  paths=[a.resolved,a.unresolved];ds=[load(x) for x in paths];errors=validate(ds,load(a.schema),entity_sets(extract_source(a.source_root)),raw_bytes=[x.read_bytes() for x in paths])
